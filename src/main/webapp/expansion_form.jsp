...
<h1>
  <c:choose>
    <c:when test="${empty expansion.id}">Nueva Expansión</c:when>
    <c:otherwise>Editar Expansión</c:otherwise>
  </c:choose>
</h1>

<form method="post"
      action="${pageContext.request.contextPath}/expansions"
  ...
  <div class="form-group">
    ...
    <c:if test="${not empty expansion.image}">
      <input type="hidden" name="existingImage" value="${expansion.image}"/>
      <p>Imagen actual:</p>
  ...
  <button type="submit" class="btn btn-primary">
    <c:choose>
      <c:when test="${empty expansion.id}">Crear</c:when>
      <c:otherwise>Actualizar</c:otherwise>
    </c:choose>
  </button>
  <a href="${pageContext.request.contextPath}/expansions"
